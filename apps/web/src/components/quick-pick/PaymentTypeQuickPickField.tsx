...
import { PaymentTypeSearchOverlay } from "@/components/money/add-payment/PaymentTypeSearchOverlay";
import { PaymentTypeQuickPickList } from "./PaymentTypeQuickPickList";
import { usePaymentTypeQuickPick } from "./usePaymentTypeQuickPick";

const DEFAULT_EXCLUDE: readonly string[] = ["STOCK_MOVEMENT"];

interface PaymentTypeQuickPickFieldProps {
  label: string;
  labelClassName: string;
  warehouseId: string;
  supabase: SupabaseClient;
  value: PaymentTypePickRow | null;
  onChange: (row: PaymentTypePickRow) => void;
  placeholder?: string;
  excludeCategories?: readonly string[];
}

export function PaymentTypeQuickPickField({
  label,
  labelClassName,
  warehouseId,
  supabase,
  ...
  excludeCategories = DEFAULT_EXCLUDE,
}: PaymentTypeQuickPickFieldProps) {
  ...
  const { query, setQuery, rows, loading, scrollRootRef, sentinelRef } = usePaymentTypeQuickPick({
    warehouseId,
    supabase,
  ...
  const committedLabel = value ? value.name : "";
