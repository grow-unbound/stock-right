...
import { insertOperationalPayment, MONEY_REFRESH_EVENT, type PaymentTypePickRow } from "@stockright/shared/api";
import {
...
import { assertIndiaMobileOptional } from "@stockright/shared/utils";
...
import { PaymentTypeQuickPickField } from "@/components/quick-pick/PaymentTypeQuickPickField";
...
export function AddPaymentForm({
  ...
  const dirty = useMemo(() => {
    ...
    return false;
  }, [paymentDate, amountStr, notes, paymentType, paymentMethod, isStaff, partyName, partyPhone]);
      ...
      <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
        <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-4 pt-2">
          <div className={fieldsLayoutClass}>
            ...
            {isStaff ?
              <div className={cn(isDetailPane && "grid grid-cols-1 gap-4 lg:col-span-2 lg:grid-cols-2 lg:gap-x-6")}>
                <div>
                ...
                <div>
                  <label className={labelClass}>Employee phone</label>
                  <input
                    type="tel"
                    inputMode="numeric"
                    value={partyPhone}
            ...
            <div className={cn(isDetailPane && "lg:col-span-2")}>
              <label htmlFor="add-payment-notes" className={labelClass}>
                Notes <span className="normal-case text-[var(--text-placeholder)]">(optional)</span>
              </label>
              <input
                id="add-payment-notes"
                type="text"
                value={notes}
