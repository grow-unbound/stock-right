-- import_staging: rental_mode_from_date uses warehouse recurring cutoff; apply_lots_from_raw calls two-arg overload.
-- Depends on: 20260501120000_rent_accrual_engine (public.rent_yearly_cutoff_in_year, warehouse_settings columns).

CREATE OR REPLACE FUNCTION import_staging.rental_mode_from_date(d date, p_warehouse_id uuid)
  ...
  SELECT COALESCE(
    (
      SELECT CASE
        ...
        WHEN d <= public.rent_yearly_cutoff_in_year(
          EXTRACT(YEAR FROM d)::integer,
          ws.yearly_rent_cutoff_month,
          ws.yearly_rent_cutoff_day
        ) THEN 'YEARLY'::public.rental_mode
        ELSE 'MONTHLY'::public.rental_mode
      END
      FROM public.warehouse_settings ws
...
DROP FUNCTION IF EXISTS import_staging.rental_mode_from_date(date);
    ...
    IF v_mode = 'IN' THEN
      ...
      IF v_lot_id IS NULL THEN
        ...
        VALUES (
          ...
          v_bags,
          v_tx,
          import_staging.rental_mode_from_date(v_tx, p_warehouse_id),
          v_loc_lot,
          CASE
...
REVOKE ALL ON FUNCTION import_staging.rental_mode_from_date(date, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION import_staging.rental_mode_from_date(date, uuid) TO service_role;
